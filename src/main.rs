...
use bevy::prelude::*;
use bevy_retrograde::core::image::{DynamicImage, GenericImageView};
...
fn setup_level(mut c: Commands, asset_server: Res<AssetServer>) {
    ...
    // Spawn camera
    c.spawn().insert_bundle(CameraBundle {
        camera: Camera {
...
fn player_vision_cone(
    ...
    let samples = 10;
    let cone_radius = 50.;
    info!("Starting ray cast");
    for n in player_q.iter() {
        info!("{:?}", n);
    }
    if let Ok(center) = player_q.single() {
        // let player = c.
        ...
        for i in 0..samples {
            ...
            info!(
                "Ray casting at {} -> {} {}",
                center.translation, pos_x, pos_y
            );

            let hit = physics_world.ray_cast_with_filter(
                center.translation,
                center.translation + Transform::from_xyz(pos_x, pos_y, 0.0).translation,
                false,
            ...
            if let Some(info) = hit {
                ...
                c.spawn()
                    .insert_bundle(ShapeBundle {
                        ...
                        global_transform: GlobalTransform::from_xyz(pos_x, pos_y, 3.),
                        ..Default::default()
                    })
                    .insert(DebugLine);

                // Draw the line Player -> collison
                c.spawn()
                    .insert_bundle(ShapeBundle {
                        shape: Shape::LineSegment {
                            points: [
                                Pos2::new(center.translation.x, center.translation.y),
                                Pos2::new(
                                    info.collision_point.x + center.translation.x,
                                    info.collision_point.y + center.translation.y,
                                ),
                            ],
                    ...
                    .insert_bundle(ShapeBundle {
                        shape: Shape::LineSegment {
                            points: [
                                Pos2::new(center.translation.x, center.translation.y),
                                Pos2::new(
                                    pos_x + center.translation.x,
                                    pos_y + center.translation.y,
                                ),
                            ],
